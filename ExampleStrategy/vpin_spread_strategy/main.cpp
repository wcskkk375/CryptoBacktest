...
namespace BacktestEngine {
    ...
    vector<Order> BacktestingEngine::OnData(JsonWrapper tick){
        ...
        double ob_coeff_value = (*indicator_operator["ob_coeff"]).CalculateIndicator(market_snapshot);
        double market_range = (*indicator_operator["market_range"]).CalculateIndicator(market_snapshot);  
        // if (current_exposure == 0){
        //     cout<<"ob_coeff_value : "<<ob_coeff_value<<endl;
        //     cout<<current_exposure<<"trade direction"<<allow_trade<<endl;
        //     cout<<"order list size"<<match_engine.OrderList.size()<<endl;
        // }
...
int main()
{
    ...
    ObImblanceIndicator obcoeffIndicator;
    IndicatorEngine * obcoeff = &obcoeffIndicator;
    be.AppendIndicator("ob_coeff", obcoeff);
